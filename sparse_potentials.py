...
def pairwise_potential_from_model_features_and_position(args, npz_path, all_positions, factors, variances, weight, device='cpu'):
    ...
    width = int(np.load(npz_path)['width']) 
    height = int(np.load(npz_path)['height']) 
    features = torch.tensor(np.load(npz_path)['features'])
...
def pairwise_potential_from_minus_model_features(args, npz_path, all_positions, features, variances, data, device='cuda'):
    ...
    # Compute squared differences
    values = batched_similarity(features, row_indexes, col_indexes, block_size=5000) 
...
def pairwise_potential_from_model_features(args, npz_path, all_positions, features, variances, data, device='cpu'):
    def batched_similarity(features, row_indexes, col_indexes, block_size=1024):
        # Validate index ranges
        values_list = []
        n = row_indexes.shape[0]
        ...
        for start in range(0, n, block_size):
            ...
            block_vals = torch.sum(rows * cols, dim=1)
            values_list.append(block_vals)
...
def pairwise_potential_from_model_features_ann(args, npz_path, all_positions, features, variances, weight, device='cpu'):
    # Access the required keys
